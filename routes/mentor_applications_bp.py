...
@jwt_required()
def apply_mentor():
    identity = get_jwt_identity()                      # JWT gives username
    user = User.query.filter_by(username=identity).first()  # ✅ find the user
    if not user:
    ...
    # ✅ Prevent duplicate pending applications
    existing = MentorApplication.query.filter_by(
        user_id=user.id, status="PENDING"               # <-- FIXED (use user.id)
    ).first()
    if existing:
        return jsonify({"error": "You already have a pending application"}), 400
...
def get_my_applications():
    ...
    apps = (
        MentorApplication.query.filter_by(user_id=user.id)  # <-- FIXED
        .order_by(MentorApplication.created_at.desc())
        .all()
    )
    return jsonify([a.to_dict() for a in apps]), 200
